...
describe('ConvertCodePipe', () => {
  ...
  class MockSanitizer extends DomSanitizer {
    public sanitize(context: SecurityContext, value: any): string {
      return value;
    }
  ...
  it('Should transform two code groups separated by dash', () => {
    expect(pipe.transform('(A12.23.1-A12.23.1)'))
      .toBe('(<span class="link" onclick="window.eonum.searchCode(\'A12.23.1\')">A12.23.1</span>' +
            '-<span class="link" onclick="window.eonum.searchCode(\'A12.23.1\')">A12.23.1</span>)');
  });
